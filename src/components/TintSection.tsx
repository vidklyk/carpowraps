...
import {
  TINT_TOTAL_FRAMES,
  TINT_MOBILE_TOTAL_FRAMES,
  TINT_SCROLL_HEIGHT,
  TINT_MOBILE_SCROLL_HEIGHT,
} from '@/lib/constants';

const CDN = 'https://pub-2bbcc9d1a232445d9e20eec38de3a44e.r2.dev';

function tintFrameUrl(i: number, mobile: boolean) {
  if (mobile) return `${CDN}/frames_tint_mobile/tint_frame_${String(i + 1).padStart(3, '0')}.jpg`;
  return `${CDN}/frames_tint/ppf_frame_${String(i + 1).padStart(3, '0')}.jpg`;
}
...
const SCROLL_OVERLAYS = [
  { from: 0.05, to: 0.33, label: 'WINDOW TINTING',           title: 'Style Meets Privacy'  },
  { from: 0.36, to: 0.63, label: 'CERAMIC TECHNOLOGY',        title: '99% IR & UV Blocked'  },
  { from: 0.66, to: 0.88, label: 'PROFESSIONAL INSTALLATION', title: 'Precision Every Time' },
];

function TintScrollAnimation() {
  ...
  const totalFramesRef = useRef(TINT_TOTAL_FRAMES);
  const scrollHeightRef = useRef(TINT_SCROLL_HEIGHT);

  const [scrollT, setScrollT]       = useState(0);
  const [entered, setEntered]       = useState(false);
  const [containerH, setContainerH] = useState(TINT_SCROLL_HEIGHT);
  ...
  useEffect(() => {
    ...
    const total  = mobile ? TINT_MOBILE_TOTAL_FRAMES : TINT_TOTAL_FRAMES;
    const height = mobile ? TINT_MOBILE_SCROLL_HEIGHT : TINT_SCROLL_HEIGHT;
    ...
    <div ref={containerRef} style={{ height: `${containerH}px` }} className="relative w-full">
      <div className="sticky top-0 w-full overflow-hidden bg-black" style={{ height: '100dvh' }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-30"
          ...
          <p className="text-white/30 text-xs tracking-[0.5em] uppercase mb-5">Window Tinting</p>
          ...
          <p className="text-white/25 text-sm mt-6 tracking-wide text-center max-w-xs px-6">
            Not just a film — protection, style and comfort every day
          </p>
        </div>
...
const TINT_BRANDS = [
  { name: 'Xpel',       sub: 'Premium Ceramic'      },
  { name: 'LLumar',     sub: 'Nano-Ceramic Series'  },
  { name: 'CarpoWraps', sub: 'Exclusive Collection' },
];

const TINT_ACCORDION = [
  {
    id: 'glare',  num: '01', title: 'Reduces Glare',
    preview: 'Harsh reflections · Safer driving',
    body: 'Eliminates harsh reflections from oncoming headlights and bright sunlight, reducing eye strain for safer, more comfortable driving in all conditions.',
  },
  {
    id: 'interior', num: '02', title: 'Protects Interior',
    preview: 'UV-induced fading · Dashboard · Upholstery',
    body: 'Shields upholstery, dashboard and trim from UV-induced fading and cracking. Your interior stays looking new far longer without costly replacement.',
  },
  {
    id: 'ir', num: '03', title: '99% IR Blocked',
    preview: 'Infrared radiation · Cooler cabin year-round',
    body: 'Nano-ceramic technology blocks up to 99% of infrared radiation — the main source of heat through glass. Your cabin stays significantly cooler without overworking the AC.',
  },
  {
    id: 'uv', num: '04', title: 'UV Protection',
    preview: '99% UV blocked · Driver & passenger safety',
    body: 'Blocks 99% of harmful UV rays protecting both driver and passengers from long-term skin damage, as well as preventing premature fading of all interior surfaces.',
  },
  {
    id: 'heat', num: '05', title: 'Heat Reduction',
    preview: 'Lower cabin temperature · Less AC load',
    body: 'Significantly lowers cabin temperature in direct sunlight. Combined with IR blocking, the difference is noticeable the moment you open the door on a hot day.',
  },
  {
    id: 'nano', num: '06', title: 'Nano-Ceramic Technology',
    preview: 'Signal-safe · Peak performance',
    body: 'Advanced ceramic nano-particles deliver maximum heat and UV rejection without metallic interference. GPS, mobile signal and radio all work perfectly through the film.',
  },
];


function TintBenefits() {
  const [openId, setOpenId] = useState<string | null>('glare');
  ...
  return (
    <section id="tint-benefits" ref={ref} className="bg-black py-16 px-6 md:px-16 lg:px-24">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12"
          ...
          <div>
            <p className="text-white/40 text-xs tracking-[0.5em] uppercase mb-3">Ceramic Film</p>
          ...
          <div className="flex items-center gap-6 md:gap-10 flex-wrap">
            {TINT_BRANDS.map((b) => (
              <div key={b.name} className="flex flex-col gap-0.5">
                <span className="text-white/70 text-base font-bold tracking-tight">{b.name}</span>
                <span className="text-white/30 text-xs tracking-wide">{b.sub}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-10"
          ...
          <p className="text-white/40 text-sm max-w-sm text-center sm:text-left">
            Each manufacturer has its own shade. Choose the one that fits your style.
          </p>
          <a href="#contact"
